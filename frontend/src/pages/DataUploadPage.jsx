    ...
    <div data-testid="data-upload-page" className="space-y-6 max-w-4xl">
      <div
        ...
        className={`border-2 border-dashed rounded-md p-10 text-center cursor-pointer transition-all duration-200 ${
          dragActive ? "border-blue-500 bg-blue-500/5" : "border-slate-700 hover:border-slate-600 bg-slate-900/50"
        }`}
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="w-14 h-14 rounded-xl bg-blue-600/10 border border-blue-500/20 flex items-center justify-center">
            <CloudArrowUp size={28} weight="duotone" className="text-blue-400" />
          </div>
          <div>
      ...
      {uploadResult && (
        <div data-testid="upload-result" className={`p-4 rounded-md border flex items-start gap-3 ${
          ...
          {uploadResult.success ? <CheckCircle size={20} className="text-emerald-400 mt-0.5" /> : <XCircle size={20} className="text-red-400 mt-0.5" />}
          <div>
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium text-slate-100" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Data Sources</h3>
      ...
      {sources.length > 0 ? (
            ...
            <TableBody>
              {sources.map((s, i) => (
                <TableRow key={i} className="border-slate-800/50 hover:bg-slate-800/30">
                  <TableCell className="text-slate-200 text-sm font-medium flex items-center gap-2">
                    {s.filename?.endsWith('.csv') ? <FileCsv size={16} className="text-blue-400" /> : <FileXls size={16} className="text-emerald-400" />}
                    {s.filename}
                  </TableCell>
                  <TableCell className="text-slate-400 text-sm">{s.row_count}</TableCell>
                  <TableCell className="text-slate-400 text-sm">{s.columns?.length || 0}</TableCell>
                  <TableCell className="text-slate-400 text-sm">{s.uploaded_at ? new Date(s.uploaded_at).toLocaleString() : '-'}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-md p-8 text-center">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-md p-5">
        <div className="flex items-center gap-2 mb-4">
          <Database size={18} weight="duotone" className="text-blue-400" />
          <h3 className="text-sm font-medium text-slate-200">Database Connection</h3>
          <span className="px-2 py-0.5 rounded text-[10px] bg-amber-500/10 text-amber-400 font-medium">Coming Soon</span>
        </div>
